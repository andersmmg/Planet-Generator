...
func _init(parent: QuadNode, direction: Vector3, terrain_manager: Node3D, leaf_index := -1):
	...
	terrain_job.connect(
		"job_finished", Callable(self, "on_patch_finished").bind(), CONNECT_DEFERRED
	)


## Update this node in the quadtree.
func visit():
	if not _viewer_node or not _terrain_manager:
		return
	var distance: float = _viewer_node.global_transform.origin.distance_to(
		_terrain_manager.global_transform.origin + _center
	)
...
func split_start():
	...
	for i in Const.MAX_LEAVES:
		...
		leaves.append(get_script().new(self, terrain.data.axis_up, _terrain_manager, i))
